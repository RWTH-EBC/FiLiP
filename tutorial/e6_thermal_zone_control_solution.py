...
com_step = 60  # communication step in seconds
        ...
        while self.current_time <= t_sim:
            if self.current_time != 0:
                ...
                t_room = self.current_output[1] + self.dt * (
                            self.kA * (t_amb - self.current_output[1]) + self.on_off*self.Q_h) / \
                         self.C_p
...
if __name__ == '__main__':
    ...
    def on_message(client, userdata, msg):
        ...
        if 'heater signal' in json_payload:
            sim_model.on_off = json_payload['heater signal']
    ...
    mqttc.subscribe(topic=topic_heater)
