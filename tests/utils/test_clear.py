...
class TestClearFunctions(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.fiware_header = FiwareHeader(
            service="filip_clear_test", service_path=settings.FIWARE_SERVICEPATH
        )
        self.cb_url = settings.CB_URL
        self.cb_client = ContextBrokerClient(
            url=self.cb_url, fiware_header=self.fiware_header
        )
        ...
        self.iota_client = IoTAClient(
            url=self.iota_url, fiware_header=self.fiware_header
        )

        self.ql_url = settings.QL_URL
        self.ql_client = QuantumLeapClient(
            url=self.ql_url, fiware_header=self.fiware_header
        )
        self.sub_dict = {
    ...
    def test_clear_context_broker(self):
        ...
        """
        entity = ContextEntity(id=str(random.randint(1, 50)), type=f"filip:object:Type")
    ...
    def test_clear_context_broker_ld(self):
        ...
        entities = [
            ContextLDEntity(id=f"urn:ngsi-ld:clear_test:{str(i)}", type="clear_test")
            for i in random_list
        ]
        self.cb_client_ld.entity_batch_operation(
        ...
        sub = SubscriptionLD(
            id=f"urn:ngsi-ld:Subscription:clear_test:{random.randint(0, 100)}",
    ...
    def test_clear_context_broker_with_url(self):
        ...
        self.assertEqual(
            0,
            len(self.cb_client.get_entity_list())
            or len(self.cb_client.get_entity_list()),
        )
    ...
    def test_clear_iot_agent(self):
        ...
        self.assertEqual(
            0,
            len(self.iota_client.get_device_list())
            or len(self.iota_client.get_group_list()),
        )
    ...
    def test_clear_quantumleap(self):
        ...
        def create_data_points():
            def create_entities(_id) -> List[ContextEntity]:
                def create_attr():
                    return {
                        "temperature": {"value": random(), "type": "Number"},
                        "humidity": {"value": random(), "type": "Number"},
                        "co2": {"value": random(), "type": "Number"},
                    }
            ...
            with QuantumLeapClient(
                url=settings.QL_URL, fiware_header=fiware_header
            ) as client:
                for i in range(rec_numbs):
                    notification_message = Message(
                        data=create_entities(i), subscriptionId="test"
                    )
        ...
        clear_quantumleap(url=self.ql_url, fiware_header=self.fiware_header)
