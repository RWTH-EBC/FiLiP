...
class TestSubscriptions(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.mqtt_topic = "".join(
            [settings.FIWARE_SERVICE, settings.FIWARE_SERVICEPATH]
        )
        ...
        self.endpoint_http = Endpoint(
            **{
                "uri": urllib.parse.urljoin(
                    str(settings.LD_CB_URL), "/ngsi-ld/v1/subscriptions"
                ),
                "accept": "application/json",
            }
        )
    ...
    def test_post_subscription_http(self):
        ...
        sub = SubscriptionLD(
            id=id, notification=notification_param, entities=[{"type": "Room"}]
        )
        self.cb_client.post_subscription(sub)
        sub_list = [
            x
            for x in self.cb_client.get_subscription_list()
    ...
    def test_get_subscription(self):
        ...
        self.assertEqual(
            sub.notification.endpoint.uri, sub_get.notification.endpoint.uri
        )
    ...
    def test_update_subscription(self):
        ...
        sub_changed = SubscriptionLD(
            id=id, notification=notification_param, entities=[{"type": "House"}]
        )
        ...
        non_sub = SubscriptionLD(
            id="urn:ngsi-ld:Subscription:nonexist",
...
class TestSubsCheckBroker(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.entity_dict = {
            ...
            "type": "Room",
            "temperature": {"type": "Property", "value": 30},
        }
        ...
        self.fiware_header = FiwareLDHeader(ngsild_tenant=settings.FIWARE_SERVICE)
        self.cb_client = ContextBrokerLDClient(
            url=settings.LD_CB_URL, fiware_header=self.fiware_header
        )
        # initial tenant
        self.cb_client.post_entity(
            ContextLDEntity(id="Dummy:1", type="Dummy"), update=True
        )
    ...
    def test_post_subscription_mqtt(self):
        ...
        def on_message(client, userdata, msg):
            ...
            self.assertEqual(
                updated_entity, json.loads(msg.payload.decode())["body"]["data"][0]
            )

        self.mqtt_client.on_message = on_message
        self.mqtt_client.connect(
            settings.LD_MQTT_BROKER_URL.host, settings.LD_MQTT_BROKER_URL.port, 60
        )
        ...
        self.cb_client.update_entity_attribute(
            entity_id="urn:ngsi-ld:Entity:test_entity03",
            attr=NamedContextProperty(type="Property", value=25, name="temperature"),
    ...
    def test_update_subscription_check_broker(self):
        ...
        def on_message(client, userdata, msg):
            ...
            self.timeout_proc.cancel()
            print(
                json.loads(msg.payload.decode())["body"]["data"][0]["temperature"][
                    "value"
                ]
            )
            self.assertEqual(
        ...
        self.cb_client.update_entity_attribute(
            entity_id="urn:ngsi-ld:Entity:test_entity03",
            attr=NamedContextProperty(
                type="Property", value=current_vals[0], name="temperature"
            ),
            attr_name="temperature",
        )
        while self.timeout_proc.is_alive():
        ...
        self.cb_client.update_entity_attribute(
            entity_id="urn:ngsi-ld:Entity:test_entity03",
            attr=NamedContextProperty(
                type="Property", value=current_vals[1], name="temperature"
            ),
            attr_name="temperature",
        )
        while self.timeout_proc.is_alive():
