    ...
    with QuantumLeapClient(url=settings.QL_URL, fiware_header=fiware_header) as client:
        ...
        for i in range(num_records):
            notification_message = Message(
                data=create_entities(), subscriptionId="test"
            )
...
class TestTimeSeries(unittest.TestCase):
    ...
    def test_insert_data(self) -> None:
        ...
        with QuantumLeapClient(
            ...
            notification_message = Message(data=entities, subscriptionId="test")
            client.post_notification(notification_message)
        time.sleep(1)
    ...
    def test_query_endpoints_by_id(self) -> None:
        ...
        with QuantumLeapClient(
            ...
            with self.assertRaises(requests.RequestException):
                client.get_entity_by_id(entity_id=entities[0].id, entity_type="MyType")
            for entity in entities:
                # get by id
                attrs_id = client.get_entity_by_id(
                    entity_id=entity.id,
                    aggr_period="minute",
                    aggr_method="avg",
    ...
    def test_query_endpoints_by_id_pattern(self) -> None:
        ...
        with QuantumLeapClient(
            ...
            if version.parse(client.get_version().get("version")) < version.parse(
                "1.0.0"
            ):
            ...
            re_patterns = {
                ".*[^mn]$": 0,  # not end with "m" or "n" -> no entities will match
        ...
        with QuantumLeapClient(
            ...
            for entity in create_entities():
                ...
                for last_n in range(
                    ...
                    self.assertGreater(last_n_records.index[0], records.index[0])
                    self.assertEqual(len(last_n_records.index), min(last_n, limit))
                ...
                for offset in range(
                    ...
                    if old_records:
                        self.assertGreater(old_records.index[0], records.index[0])
                    old_records = records
