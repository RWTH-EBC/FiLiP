...
class TestContextv2IoTModels(unittest.TestCase):
    ...
    def test_fiware_safe_fields(self):
        ...
        for string in valid_strings:
            IoTABaseAttribute(
                name=string, type=string, entity_name=string, entity_type=string
            )
        ...
        for string in special_strings:
            IoTABaseAttribute(
                name="name", type="name", entity_name=string, entity_type=string
            )
            ServiceGroup(entity_type=string, resource="", apikey="")
            Device(
                device_id="",
    ...
    def test_expression_language(self):
        ...
        device1 = Device(
            ...
            expressionLanguage=ExpressionLanguage.JEXL,
            attributes=[
                DeviceAttribute(name="value", type="Number"),
                DeviceAttribute(
                    name="fraction", type="Number", expression="(value + 3) / 10"
                ),
                DeviceAttribute(name="spaces", type="Text"),
                DeviceAttribute(name="trimmed", type="Text", expression="spaces|trim"),
            ],
        )
        ...
        mqtt_cl.publish(
            topic=f"/json/{api_key}/{device1.device_id}/attrs",
        ...
        attr3 = DeviceAttribute(
            name="brackets", type="Number", expression="((2 + 3) / 10"
        )
        with self.assertRaises(pyjexl.jexl.ParseError):
        ...
        self.assertEqual(
            service_group_null_expression.expressionLanguage, ExpressionLanguage.JEXL
        )

        device4 = Device(
            device_id="null_expression_device",
    ...
    def test_add_device_attributes(self):
        ...
        def initial_device():
            attr = DeviceAttribute(name="temperature", type="Number")
            return Device(
                device_id="dummy:01",
        ...
        attr1 = DeviceAttribute(name="temperature", type="Number")
        ...
        attr2 = DeviceAttribute(
            name="temperature", type="Number", object_id="temperature"
        )
        with self.assertRaises(ValueError):
        ...
        attr3 = DeviceAttribute(name="temperature", type="Number", object_id="t1")
        device_c.add_attribute(attribute=attr3)
        # success
        attr4 = DeviceAttribute(name="temperature", type="Number", object_id="t2")
        device_c.add_attribute(attribute=attr4)
        # fail, because object id is duplicated
        attr5 = DeviceAttribute(name="temperature2", type="Number", object_id="t2")
        with self.assertRaises(ValueError):
    ...
    def test_device_creation(self):
        ...
        def create_device(
            attr1_name, attr2_name, attr1_object_id=None, attr2_object_id=None
        ):
            _attr1 = DeviceAttribute(
                name=attr1_name, object_id=attr1_object_id, type="Number"
            )
            _attr2 = DeviceAttribute(
                name=attr2_name, object_id=attr2_object_id, type="Number"
            )
            return Device(
                device_id="dummy:01",
                entity_name="entity:01",
