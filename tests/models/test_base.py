...
class TestModels(unittest.TestCase):
    ...
    def test_fiware_header(self):
        ...
        self.assertEqual(header.model_dump(by_alias=True), self.fiware_header)
        self.assertEqual(
        ...
        with ContextBrokerClient(url=settings.CB_URL, fiware_header=headers) as client:
            ...
            self.assertGreaterEqual(
                len(client.get_entity_list()), len(self.service_paths)
            )
            for path in self.service_paths:
                client.fiware_service_path = path
                client.delete_entity(entity_id=entity.id, entity_type=entity.type)
    ...
    def test_strings_in_models(self) -> None:
        ...
        def field_value_tests(
            dictionary: dict, keychain: List[str], test_keys: bool = False
        ):
            ...
            for field, value in dictionary.items():
                if isinstance(value, dict):
                    ...
                    for test_char, needs_to_succeed in [
                        ("'", False),
                        ('"', False),
                        ("", True),
                    ]:
        ...
        header = FiwareHeader.model_validate(self.fiware_header)
        with ContextBrokerClient(url=settings.CB_URL, fiware_header=header) as client:
    ...
    def tearDown(self) -> None:
        ...
        with ContextBrokerClient(url=settings.CB_URL) as client:
            ...
            for path in self.service_paths:
                header = FiwareHeader(
                    service=settings.FIWARE_SERVICE, service_path=path
                )
