...
class TestSettings(BaseSettings):
    ...
    LOG_LEVEL: LogLevel = Field(
        default=LogLevel.ERROR, validation_alias=AliasChoices("LOG_LEVEL", "LOGLEVEL")
    )
    ...
    IOTA_URL: AnyHttpUrl = Field(
        default="http://localhost:4041", validation_alias="IOTA_URL"
    )
    IOTA_JSON_URL: AnyHttpUrl = Field(
        default="http://localhost:4041", validation_alias="IOTA_JSON_URL"
    )

    IOTA_UL_URL: AnyHttpUrl = Field(
        default="http://127.0.0.1:4061", validation_alias=AliasChoices("IOTA_UL_URL")
    )
    ...
    LD_MQTT_BROKER_URL_INTERNAL: AnyUrl = Field(
        default="mqtt://mqtt-broker-ld:1883",
        validation_alias=AliasChoices(
            "LD_MQTT_BROKER_URL_INTERNAL", "LD_MQTT_URL_INTERNAL"
        ),
    )

    # IF CI_JOB_ID is present it will always overwrite the service path
    CI_JOB_ID: Optional[str] = Field(
        default=None, validation_alias=AliasChoices("CI_JOB_ID")
    )
    ...
    FIWARE_SERVICEPATH: str = Field(
        default_factory=generate_servicepath,
        validation_alias=AliasChoices(
            "FIWARE_PATH", "FIWARE_SERVICEPATH", "FIWARE_SERVICE_PATH"
        ),
    )
