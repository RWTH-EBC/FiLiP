...
READ_SUBSCRIPTIONS_FILEPATH = Path(
...
if __name__ == "__main__":
    ...
    cbc.post_subscription(
        subscription=Subscription(
            subject=Subject(
                **{"entities": [{"id": heater.entity_name, "type": heater.entity_type}]}
            ),
            notification=Notification(
                **{"http": {"url": "http://quantumleap:8668/v2/notify"}}
            ),
    ...
    history_zone_temperature_sensor = history_zone_temperature_sensor.to_pandas()
    ...
    history_heater = qlc.get_entity_by_id(
        entity_id=heater.entity_name, entity_type=heater.entity_type, last_n=10000
    )
