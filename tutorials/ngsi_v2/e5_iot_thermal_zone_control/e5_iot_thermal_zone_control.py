...
WRITE_GROUPS_FILEPATH = Path("../e5_iot_thermal_zone_control_groups.json")
WRITE_DEVICES_FILEPATH = Path("../e5_iot_thermal_zone_control_devices.json")
WRITE_SUBSCRIPTIONS_FILEPATH = Path("../e5_iot_thermal_zone_control_subscriptions.json")
# path to read json-files from previous exercises
READ_GROUPS_FILEPATH = Path("../e4_iot_thermal_zone_sensors_groups.json")
READ_DEVICES_FILEPATH = Path("../e4_iot_thermal_zone_sensors_devices.json")
...
if __name__ == "__main__":
    ...
    print(
        f"Your device entity before running the simulation: \n "
    ...
    def on_command(client, obj, msg):
        ...
        client.publish(device_id=device_id, command_name=..., payload=...)

    # ToDo: Add the command callback to your MQTTClient. This will get
    #  triggered for the specified device_id.
    mqttc.add_command_callback(device_id=..., callback=...)
    ...
    assert (
        WRITE_SUBSCRIPTIONS_FILEPATH.suffix == ".json"
