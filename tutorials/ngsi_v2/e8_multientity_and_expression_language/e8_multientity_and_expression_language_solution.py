...
if __name__ == "__main__":
    ...
    device2 = Device(
        ...
        expressionLanguage=ExpressionLanguage.JEXL,
        attributes=[
            DeviceAttribute(name="value", type="Number", expression="5 * value"),
            DeviceAttribute(name="spaces", type="Text"),
            DeviceAttribute(name="consumption", type="Text", expression="spaces|trim"),
            DeviceAttribute(name="timestamp", type="Number"),
            DeviceAttribute(
                name="iso_time", type="Text", expression="timestamp|toisodate"
            ),
        ],
    )
    ...
    entity1 = ContextEntity(
        id="urn:ngsi-ld:SubWeatherStation:001", type="SubWeatherStation"
    )
    entity1.add_attributes(attrs=[NamedContextAttribute(name="vol", type="Number")])
    cb_client.post_entity(entity1)

    entity2 = ContextEntity(
        id="urn:ngsi-ld:SubWeatherStation:002", type="SubWeatherStation"
    )
    ...
    device3 = Device(
        ...
        attributes=[
            ...
            DeviceAttribute(
                object_id="v", name="vol", type="Number", expression="100 * v"
            ),
        ],
    )
    ...
    client.publish(
        topic=f"/json/{APIKEY}/{device3.device_id}/attrs",
    ...
    # Printing context entities of OCB
    for context_entity in cb_client.get_entity_list(
        entity_types=["WeatherStation", "SubWeatherStation"]
    ):
        print(context_entity.model_dump_json(indent=4))
