...
if __name__ == "__main__":
    ...
    service_group_json = ServiceGroup(
        apikey=SERVICE_PATH.strip("/"), resource="/iot/json"
    )
    service_group_ul = ServiceGroup(apikey=SERVICE_PATH.strip("/"), resource="/iot/d")
    ...
    mqttc.message_callback_add(sub=first_topic, callback=on_message_first)
    mqttc.message_callback_add(sub=second_topic, callback=on_message_second)
    mqtt_broker_url = urlparse(MQTT_BROKER_URL)

    mqttc.connect(
        host=mqtt_broker_url.hostname,
    ...
    mqttc.add_command_callback(device_id=device_json.device_id, callback=on_command)

    from filip.clients.ngsi_v2 import HttpClient, HttpClientConfig

    httpc_config = HttpClientConfig(cb_url=CB_URL, iota_url=IOTA_URL)
    httpc = HttpClient(fiware_header=fiware_header, config=httpc_config)
    ...
    entity = httpc.cb.get_entity(
        entity_id=device_json.device_id, entity_type=device_json.entity_type
    )
    context_command = NamedCommand(name=device_json.commands[0].name, value=False)
    ...
    payload = random.randint(0, 30)
    mqttc.publish(
        device_id=device_json.device_id, attribute_name="temperature", payload=payload
    )
    time.sleep(1)
    entity = httpc.cb.get_entity(
        entity_id=device_json.device_id, entity_type=device_json.entity_type
