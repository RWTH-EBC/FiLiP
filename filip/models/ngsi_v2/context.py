...
class ContextEntity(ContextEntityKeyValues):
    ...
    def get_commands(
            ...
            -> Union[List[NamedContextAttribute], Dict[str, ContextAttribute]]:
        """
        Get all commands of the context entity
        ...
        """
        return self.get_attributes(whitelisted_attribute_types=[
            DataType.COMMAND], response_format=response_format)
    ...
    def get_command_triple(self, command_attribute_name: str) -> \
        ...
        Raises:
            ...
            AttributeError, if the command was not autogenerated from Fiware,
                meaning the the attributes "name"_status or "name"_info do not
                exist or have wrong types
        Returns:
            (Command, Command_status, Command_info)
        """

        command = self.get_attribute(command_attribute_name)
        if not command.type == DataType.COMMAND:
            raise KeyError

        try:
            command_status = \
                self.get_attribute(f'{command_attribute_name}_status')
            if not command_status.type == DataType.COMMAND_STATUS:
        ...
        try:
            command_info = \
                self.get_attribute(f'{command_attribute_name}_info')
            if not command_info.type == DataType.COMMAND_RESULT:
                raise AttributeError
        except KeyError:
            raise AttributeError
