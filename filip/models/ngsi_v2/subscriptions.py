...
class NgsiPayload(BaseModel):
    ...
    id: Optional[str] = Field(default=None, max_length=256, min_length=1, frozen=True)
        ...
        assert (
            len(
                [
                    ...
                    if ((v is not None) and (k in ["payload", "ngsi", "json"]))
                ]
            )
            <= 1
        )
...
class Condition(BaseModel):
    ...
    attrs: Optional[Union[str, List[str]]] = Field(
        default=None, description="array of attribute names"
    )
    ...
    alterationTypes: Optional[List[str]] = Field(
        default=None, description="list of alteration types triggering the subscription"
    )
        ...
        if v is None:
            ...
            for item in v:
                if item not in allowed_types:
                    raise ValueError(
                        f"{item} is not a valid alterationType"
...
class Subject(BaseModel):
    ...
    entities: List[EntityPattern] = Field(
        description="A list of objects, each one composed of by an Entity " "Object:"
    )
...
class Subscription(BaseModel):
    ...
    description: Optional[str] = Field(
        default=None,
        description="A free text used by the client to describe the " "subscription.",
    )
